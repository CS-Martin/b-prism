class MapboxApiService {
    ...
    private async tryWaypointRouting(start: [number, number], destination: [number, number], damagedRoads: any, profile: string): Promise<any> {
        ...
        const offsets = [0.02, 0.03, 0.04, 0.05]; // Larger offsets
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data.code === 'Ok' && data.routes.length > 0) {
                            ...
                            // Check if this waypoint route avoids damaged roads
                            const isSafe = this.checkRouteSafety(route.geometry.coordinates, damagedRoads);
                            
                            if (isSafe) {
                                console.log('Found safe waypoint route with offset:', offset);
                                return {
                                    type: 'Feature' as const,
                                    geometry: route.geometry,
                                    properties: {
    ...
    private checkRouteSafety(routeCoordinates: any[], damagedRoads: any): boolean {
        for (const damagedRoad of damagedRoads) {
            const geometry = typeof damagedRoad.geometry === 'string' ? JSON.parse(damagedRoad.geometry) : damagedRoad.geometry;
            const damagedCoords = geometry.coordinates;
