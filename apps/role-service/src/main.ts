...
async function bootstrap() {
    ...
    const config = new DocumentBuilder().setTitle('Role Service').setDescription('Role Service API').setVersion('1.0').build();

    const document = SwaggerModule.createDocument(app, config);

    SwaggerModule.setup('api', app, document, { useGlobalPrefix: true });

    const port = process.env.NEXT_PUBLIC_ROLE_SERVICE_API_PORT;
